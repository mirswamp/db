...
begin
    ...
    if metric_tool_in = 'Y' then
      ...
      insert into tool_shed.tool_version (tool_version_uuid, tool_uuid, version_no, version_string, comment_public, tool_path, checksum)
        values (tool_version_uuid_in,tool_uuid_in,version_no_in,version_string_in,comment_public_in,tool_path_in,checksum_in)
         on duplicate key update tool_uuid = tool_uuid_in,
                                 version_no = version_no_in,
                                 version_string = version_string_in,
                                 comment_public = comment_public_in,
                                 tool_path = tool_path_in,
                                 checksum = checksum_in;
    end if;
...
call insert_tool(@tool_uuid,@tool_owner_uuid,@name,@description,@tool_sharing_status,@policy_code,@tool_version_uuid,@version_no,@version_string,@comment_public,@tool_path,@checksum,@metric_tool);
