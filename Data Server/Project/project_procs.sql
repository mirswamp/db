...
CREATE TRIGGER class_user_BINS BEFORE INSERT ON class_user FOR EACH ROW
  BEGIN
    ...
    DECLARE user_info_var VARCHAR(5000);
    DECLARE end_date_var DATETIME;
    DECLARE end_of_loop BOOL;
    DECLARE cur1 CURSOR FOR
      select permission_code, user_info
        from project.class_auto_approve_permission
    ...
    OPEN cur1;
    read_loop: LOOP
      FETCH cur1 INTO permission_code_var, user_info_var;
      IF end_of_loop IS TRUE THEN
        LEAVE read_loop;
      END IF;

      # Does this permission also require a policy?
      select policy_code
        into policy_code_var #declare
        from project.permission
       where permission_code = permission_code_var;

      # get class end date for permission expiration
      select end_date
        into end_date_var #declare
        from project.class
       where class_code = NEW.class_code;
      ...
      insert into project.user_permission (
         ...
         values (
           ...
           permission_code_var, #permission_code,
           NEW.user_uid, #user_uid,
           CONCAT('Student in Class ',NEW.class_code), #user_comment,
           'auto-approved for class enrollment', #admin_comment,
           user_info_var, #meta_information,
           now(), #request_date,
           now(), #grant_date,
           end_date_var  #expiration_date
           );
      if policy_code_var is not null then
        delete from project.user_policy
         where policy_code = policy_code_var
           and user_uid = NEW.user_uid;
      ...
      DO (SELECT 'nothing' FROM project WHERE FALSE);
  END;
$$

CREATE TRIGGER class_user_BDEL BEFORE DELETE ON class_user FOR EACH ROW
  BEGIN
    if OLD.class_code in (select class_code from class_auto_approve_permission) then
      update project.user_permission
         set delete_date = now(),
             grant_date = null,
             denial_date = null,
             expiration_date = null,
             user_comment = concat('Student withdrawn from Class ',OLD.class_code),
             admin_comment = 'auto-revoked for class withdrawal'
       where permission_code in (select permission_code
                                   from class_auto_approve_permission
                                  where class_code = OLD.class_code)
         and user_uid = OLD.user_uid;

      delete from project.user_policy
       where policy_code in (select p.policy_code
                               from permission p
                              inner join class_auto_approve_permission caap on caap.permission_code = p.permission_code
                              where caap.class_code = OLD.class_code)
         and user_uid = OLD.user_uid;
    end if;
  END;
$$
