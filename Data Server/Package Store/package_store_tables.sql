...
CREATE TABLE package_version (
  ...
  build_file             VARCHAR(1000)                        COMMENT '',
  build_system           VARCHAR(200)                        COMMENT 'specify build system',
  build_cmd              VARCHAR(4000)                        COMMENT 'populated only if build_system is other',
  build_target           VARCHAR(200)                        COMMENT '',
  build_dir              VARCHAR(1000) NULL DEFAULT '.'       COMMENT 'path within the package where the build step should occur',
  build_opt              VARCHAR(4000) NULL DEFAULT NULL      COMMENT 'specifies additional options to pass to the build tool that may be package specific',
  config_cmd             VARCHAR(4000) NULL DEFAULT NULL     COMMENT 'command to run to configure a package prior to building the package',
  config_opt             VARCHAR(4000) NULL DEFAULT NULL      COMMENT 'options to provide to the config_cmd',
  config_dir             VARCHAR(1000) NULL DEFAULT '.'       COMMENT 'path where the configure step should occur within the package tree',
